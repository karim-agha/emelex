...
/**
 * Emelex regression seam: make the next Metal completion callback report a
 * synthetic failure. This is not a stable public mlx-c API.
 */
void mlx_emelex_test_inject_metal_completion_failure(void);
/**
 * Returns the default stream on the given device.
 */
int mlx_get_default_stream(mlx_stream* stream, mlx_device dev);
/**
 * Writes the current default CPU stream into `stream`.
 */
int mlx_get_default_cpu_stream(mlx_stream* stream);
/**
 * Set default stream.
 */
