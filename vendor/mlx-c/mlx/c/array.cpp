...
std::optional<mlx::core::Stream> evaluation_stream(
    const mlx::core::array& array) {
  if (array.has_primitive()) {
    return array.primitive().stream();
  }
  if (array.event().valid()) {
    return array.event().stream();
  }
  return std::nullopt;
}
...
extern "C" int mlx_array_eval(mlx_array arr) {
  try {
    auto& array = mlx_array_get_(arr);
    auto stream = evaluation_stream(array);
    array.eval();
    if (stream) {
      // Emelex patch: asynchronous backend errors are retained by their
      // scheduler/framework boundary and rethrown only from caller-thread
      // synchronization. CPU Load failures need this just as Metal does.
