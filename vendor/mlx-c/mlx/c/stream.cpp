...
extern "C" void mlx_emelex_test_inject_metal_completion_failure(void) {
  mlx::core::metal::inject_command_buffer_completion_failure_for_test();
}
...
extern "C" int mlx_get_default_cpu_stream(mlx_stream* stream) {
  try {
    mlx_stream_set_(
        *stream, mlx::core::default_stream(mlx::core::Device::DeviceType::cpu));
    return 0;
  } catch (std::exception& e) {
    mlx_error("%s", e.what());
    return 1;
