...
TEST_CASE("test synchronize reports only pre-barrier CPU exceptions") {
  auto stream = new_stream(Device::cpu);

  synchronize(stream);
  scheduler::enqueue(stream, []() {
    throw std::runtime_error("post-barrier scheduler failure");
  });

  CHECK_THROWS_WITH(
      synchronize(stream), "post-barrier scheduler failure");
  CHECK_NOTHROW(synchronize(stream));
}

TEST_CASE("test CPU exception quarantines dependent tasks until synchronize") {
  auto stream = new_stream(Device::cpu);
  std::atomic<bool> dependent_ran{false};

  scheduler::enqueue(stream, []() {
    throw std::runtime_error("scheduler dependency failure");
  });
  scheduler::enqueue(stream, [&dependent_ran]() { dependent_ran = true; });

  CHECK_THROWS_WITH(synchronize(stream), "scheduler dependency failure");
  CHECK_FALSE(dependent_ran.load());
  CHECK(scheduler::n_active_tasks() == 0);

  scheduler::enqueue(stream, [&dependent_ran]() { dependent_ran = true; });
  CHECK_NOTHROW(synchronize(stream));
  CHECK(dependent_ran.load());
}
