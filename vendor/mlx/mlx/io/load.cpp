...
// emelex patch: shared exact positional-read loop. Keeping offset advancement
// here makes short-read behavior directly regression-testable.
...
bool pread_all(
    ...
    char* buffer) {
  while (size != 0) {
    auto m = read_at(buffer, size, offset);
    if (m <= 0) {
      return false;
    }
    buffer += m;
...
void ParallelFileReader::read(char* data, size_t n, size_t offset) {
  auto readfn = [fd = fd_](size_t offset, size_t size, char* buffer) -> bool {
    return pread_all(
        [fd](char* destination, size_t remaining, size_t position) {
          return pread(fd, destination, remaining, position);
        },
        offset,
        size,
        buffer);
...
// emelex patch: deterministic native seam forces three short reads and proves
// the positional offset advances alongside the destination pointer.
extern "C" int mlx_emelex_test_partial_pread_offsets(void) noexcept {
  try {
    constexpr char source[] = "abcdef";
    char output[5] = {};
    std::vector<size_t> offsets;
    auto read_at = [&offsets, &source](
                       char* destination,
                       size_t remaining,
                       size_t offset) -> int64_t {
      offsets.push_back(offset);
      const auto count = std::min<size_t>(remaining, 2);
      std::memcpy(destination, source + offset, count);
      return static_cast<int64_t>(count);
    };
    if (!mlx::core::pread_all(read_at, 1, sizeof(output), output)) {
      return 1;
    }
    return std::memcmp(output, "bcdef", sizeof(output)) == 0 &&
            offsets == std::vector<size_t>{1, 3, 5}
        ? 0
        : 1;
  } catch (...) {
    return 1;
...
// emelex patch: subprocess-only seam verifies descriptor zero is accepted and
// closed by both reader and writer wrappers.
extern "C" int mlx_emelex_test_fd_zero_io(void) noexcept {
  int saved_stdin = -1;
  char path[] = "/tmp/emelex-mlx-io-XXXXXX";
  try {
    int seed = mkstemp(path);
    if (seed < 0) {
      return 1;
    }
    constexpr char input[] = "fd-zero";
    if (::write(seed, input, sizeof(input)) !=
        static_cast<ssize_t>(sizeof(input))) {
      close(seed);
      unlink(path);
      return 1;
    }
    ...
    close(STDIN_FILENO);
    {
      mlx::core::io::ParallelFileReader reader(path);
      if (!reader.is_open()) {
        throw std::runtime_error("descriptor-zero reader did not open");
      }
      char readback[sizeof(input)] = {};
      reader.read(readback, sizeof(readback));
      if (std::memcmp(readback, input, sizeof(input)) != 0) {
        throw std::runtime_error("descriptor-zero reader returned wrong bytes");
      }
    }
    {
      mlx::core::io::FileWriter writer(path);
      if (!writer.is_open()) {
        throw std::runtime_error("descriptor-zero writer did not open");
      }
      constexpr char output[] = "writer-zero";
      writer.write(output, sizeof(output));
    }
    unlink(path);
    if (saved_stdin >= 0) {
