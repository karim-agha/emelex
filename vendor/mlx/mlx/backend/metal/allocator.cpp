...
Buffer MetalAllocator::malloc(size_t size) {
  ...
  if (!buf) {
    size_t mem_required =
        active_memory_ + buffer_cache_.cache_size() + size;
    ...
    // Keep admission and resource creation in one critical section. Otherwise
    // concurrent callers can all pass the limit check before any increments
    // num_resources_.
  ...
  const auto cache_memory = buffer_cache_.cache_size();
  if (cache_memory > max_pool_size_) {
    num_resources_ -= buffer_cache_.release_cached_buffers(
        cache_memory - max_pool_size_);
  }
...
Buffer MetalAllocator::make_buffer(void* ptr, size_t size) {
  ...
  // Serialize no-copy resource creation with malloc's admission check. A null
  // result deliberately lets array construction fall back to malloc.
