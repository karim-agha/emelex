...
class MLX_API CommandEncoder {
 ...
 private:
  ...
  Device& device_;
  std::shared_ptr<AsyncCommandBufferErrors> async_errors_;
  std::shared_ptr<CommandEncoderOutputState> output_state_;
...
class MLX_API Device {
 ...
 private:
  ...
  // When both caches are needed, acquire library_mtx_ before kernel_mtx_.
...
MLX_API void set_default_library_path(const std::string& path);
...
// Emelex regression seam. Arms exactly one command-buffer completion callback
// to throw internally; the callback must catch and report it through the next
// caller-thread synchronization.
