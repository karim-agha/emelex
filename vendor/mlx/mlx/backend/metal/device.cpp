...
struct AsyncCommandBufferFailure {
  NS::SharedPtr<NS::Error> error;
  bool failed_without_error{false};

  explicit operator bool() const {
    return error || failed_without_error;
  }
};

struct AsyncCommandBufferErrors {
  std::mutex mutex;
  std::condition_variable completion;
  size_t pending{0};
  NS::SharedPtr<NS::Error> first_error;
  bool failed_without_error{false};

  AsyncCommandBufferFailure wait_and_take() {
    std::unique_lock lock(mutex);
    completion.wait(lock, [this] { return pending == 0; });
    AsyncCommandBufferFailure failure{std::move(first_error),
                                      failed_without_error};
    first_error.reset();
    failed_without_error = false;
    return failure;
  }
};

struct CommandEncoderOutputState {
  std::mutex mutex;
  std::unordered_map<const void*, NS::SharedPtr<MTL::Fence>> previous_outputs;
};

namespace {

std::mutex default_library_path_mutex;
std::optional<std::string> default_library_path;
bool default_library_loaded = false;
std::atomic_bool inject_completion_failure{false};

struct InjectedCompletionFailure {};

void settle_command_buffer(
    const std::shared_ptr<AsyncCommandBufferErrors>& errors,
    const std::shared_ptr<std::atomic_bool>& settled,
    NS::SharedPtr<NS::Error> error = {},
    bool failed_without_error = false) noexcept {
  ...
  try {
    {
      ...
      if (!errors->first_error && !errors->failed_without_error) {
        errors->first_error = std::move(error);
        errors->failed_without_error = failed_without_error;
      }
      if (errors->pending > 0) {
        errors->pending--;
      }
    }
    errors->completion.notify_all();
  } catch (...) {
    // A valid std::mutex and SharedPtr move are non-failing in practice.
    // The catch is the final framework-thread boundary: no C++ exception may
    // escape a Metal completion callback.
    errors->completion.notify_all();
  }
}

std::string command_buffer_failure_message(
    const AsyncCommandBufferFailure& failure) {
  if (failure.error) {
    auto* description = failure.error->localizedDescription();
  ...
  return "[METAL] Command buffer execution failed without an NSError.";
}
...
MTL::Library* load_default_library(MTL::Device* device) {
  std::optional<std::string> configured_path;
  {
    std::lock_guard lock(default_library_path_mutex);
    default_library_loaded = true;
    configured_path = default_library_path;
  }
  if (configured_path) {
    auto [configured, error] =
        load_library_from_path(device, configured_path->c_str());
    if (configured) {
      return configured;
    }
    std::ostringstream msg;
    msg << "Failed to load the configured default metallib from <"
        << *configured_path << ">";
    if (error != nullptr) {
...
CommandEncoder::CommandEncoder(
    ...
    : device_(d),
      async_errors_(std::make_shared<AsyncCommandBufferErrors>()),
      output_state_(std::make_shared<CommandEncoderOutputState>()) {
  auto pool = new_scoped_memory_pool();
...
void CommandEncoder::end_encoding() {
  ...
  auto output_state = output_state_;
  {
    std::lock_guard lk(output_state->mutex);
    for (auto& in : all_inputs_) {
      if (auto it = output_state->previous_outputs.find(in);
          it != output_state->previous_outputs.end()) {
    ...
    for (auto& out : all_outputs_) {
      output_state->previous_outputs[out] = fence_;
    }
  }

  encoder_->updateFence(fence_.get());
  buffer_->addCompletedHandler([output_state = std::move(output_state),
                                fence = std::move(fence_),
                                temporaries = std::move(temporaries_),
                                all_outputs = std::move(all_outputs_),
                                waiting_on = std::move(waiting_on)](
                                   MTL::CommandBuffer*) mutable noexcept {
    try {
      std::lock_guard lk(output_state->mutex);
      for (auto& o : all_outputs) {
        if (auto it = output_state->previous_outputs.find(o);
            it != output_state->previous_outputs.end()) {
          if (it->second == fence) {
            output_state->previous_outputs.erase(it);
          }
        }
      }
    } catch (...) {
      // This callback owns only best-effort bookkeeping. Never unwind
      // through Metal's framework thread.
    }
  });
...
bool CommandEncoder::needs_commit() const {
  ...
  const bool exceeds_mb =
      max_mb >= 0 &&
      (buffer_sizes_ >> 20) > static_cast<std::size_t>(max_mb);
  return (buffer_ops_ > max_ops) || exceeds_mb;
}
...
void CommandEncoder::commit() {
  ...
  try {
    buffer_->addCompletedHandler([errors = std::move(errors),
                                  settled](MTL::CommandBuffer*
                                               command_buffer) noexcept {
      NS::SharedPtr<NS::Error> error;
      bool failed_without_error = false;
      try {
        if (inject_completion_failure.exchange(
                false, std::memory_order_acq_rel)) {
          throw InjectedCompletionFailure{};
        }
        if (command_buffer == nullptr) {
          failed_without_error = true;
        } else if (command_buffer->status() == MTL::CommandBufferStatusError) {
          if (auto* command_buffer_error = command_buffer->error();
              command_buffer_error != nullptr) {
            error = NS::RetainPtr(command_buffer_error);
          } else {
            failed_without_error = true;
          }
        }
      ...
      settle_command_buffer(
          errors, settled, std::move(error), failed_without_error);
    });
    buffer_->commit();
  } catch (...) {
    settle_command_buffer(async_errors_, settled);
    throw;
  }
  buffer_ = NS::RetainPtr(queue_->commandBufferWithUnretainedReferences());
  buffer_ops_ = 0;
...
void CommandEncoder::synchronize_and_rethrow() {
  auto pool = new_scoped_memory_pool();
  end_encoding();
  commit();
  auto failure = async_errors_->wait_and_take();
  if (failure) {
    throw std::runtime_error(command_buffer_failure_message(failure));
  }
}

void CommandEncoder::drain_noexcept() noexcept {
  try {
    auto pool = new_scoped_memory_pool();
    end_encoding();
    commit();
  ...
  try {
    static_cast<void>(async_errors_->wait_and_take());
  } catch (...) {
  }
}
...
MTL::ComputePipelineState* Device::get_kernel(
  ...
  // Library cache entries are immutable and live for the Device lifetime.
  std::shared_lock library_lock(library_mtx_);
  {
...
void set_default_library_path(const std::string& path) {
  if (path.empty()) {
    throw std::invalid_argument("default metallib path cannot be empty");
  }
  std::lock_guard lock(default_library_path_mutex);
  if (default_library_loaded) {
    throw std::logic_error(
        "default metallib path cannot change after Metal initialization");
  }
  if (default_library_path && *default_library_path != path) {
    throw std::logic_error(
        "default metallib path was already configured for this process");
  }
...
void inject_command_buffer_completion_failure_for_test() noexcept {
  inject_completion_failure.store(true, std::memory_order_release);
}
