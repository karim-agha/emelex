...
void CustomKernel::eval_gpu(
  ...
  // Metal command buffers use unretained resource references, so compiled
  // libraries and pipelines must outlive every submitted command. Keying the
  // process-lifetime Device cache by a canonical name/source tuple avoids
  // mutable replacement and keeps those objects strongly owned.
  const auto library_key =
      std::to_string(name_.size()) + ":" + name_ + source_;
  auto lib =
      d.get_library(library_key, [this] { return metal::utils() + source_; });
  auto kernel = d.get_kernel(name_, lib);
  compute_encoder.set_compute_pipeline_state(kernel);
  const auto max_tg_size = kernel->maxTotalThreadsPerThreadgroup();
