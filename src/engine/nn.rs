...
use crate::engine::{
	...
	error::{Error, Result},
	ops,
	quant::{QuantParams, Quantization},
};
...
impl WeightMap {
	...
	/// Borrow a tensor without consuming it (read-only shape/dtype access).
	///
	/// emelex patch (not upstream): non-consuming inspection API for MTP
	/// detection. All pre-sanitize detection (raw-HF predicate, prefix probe,
	/// sentinel/gate-shape validation) uses `peek`
	/// exclusively — detection is contractually non-mutating (no
	/// take/reinsert while probing).
	pub fn peek(&self, key: &str) -> Option<&Array> {
	...
	pub fn normalize_quant_keys(&mut self, f: impl Fn(&str) -> Option<String>) {
		let entries: Vec<(String, crate::engine::quant::LayerOverride)> =
...
impl Linear {
	...
	pub fn weight_dtype(
		&self,
		input_dtype: crate::engine::array::Dtype,
	) -> crate::engine::array::Dtype {
		match self {
