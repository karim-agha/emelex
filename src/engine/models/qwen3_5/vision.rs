...
//! space. Reuses [`crate::engine::media::image::ProcessedImage`] (the same
...
use super::super::config::{get_i32, optional_i32};
use crate::engine::{
	array::Array,
	error::{Error, Result},
...
impl Qwen35VisionConfig {
	pub fn from_json(cfg: &Value) -> Result<Self> {
		...
		let num_layers = match optional_i32(cfg, "depth")? {
			Some(value) => value,
			None => get_i32(cfg, "num_hidden_layers", 12)?,
		};
...
impl Qwen35VisionTower {
	pub fn load(weights: &mut WeightMap, config: Qwen35VisionConfig) -> Result<Self> {
		...
		let patch_embed = Linear::Dense(crate::engine::nn::DenseLinear {
			weight: pe_flat,
			bias: pe_bias,
		});
	...
	fn rotary_cos_sin(&self, patch_h: i32, patch_w: i32, head_dim: i32) -> Result<(Array, Array)> {
		...
		let positions = ops::arange(
			0.0,
			max_grid as f64,
			1.0,
			crate::engine::array::Dtype::Float32,
		)?;
...
fn bilinear_interpolate_grid(grid: &Array, target_h: i32, target_w: i32) -> Result<Array> {
	...
	let interp_matrix = |target: i32, base: i32| -> Result<Array> {
	...
	// Interpolate along H: [target_h, base_h] @ [base_h, base_w * dim] ->
	// [target_h, base_w * dim]
