...
use super::{
	base::{forward_embeds_chunked, splice_media_features},
	cache::{KvCache, LayerCache},
	config::{get_bool, get_f32, get_i32, optional_f32, optional_i32, require_i32, text_config},
};
use crate::engine::{
	Cancellation,
	array::Array,
...
/// KV a shared-tail layer borrows from its owning layer: keys, values,
/// the absolute query offset, and the retained keys' start position
/// (nonzero once a windowed sliding cache has trimmed).
type SharedKv = (Array, Array, i32, i32);
...
impl Gemma4Config {
	pub fn from_json(root: &Value) -> Result<Self> {
			...
			Some(Value::Array(arr)) => arr
				...
				.map(|v| match v.as_str() {
					...
					Some(value) => Err(Error::Config(format!(
						"unsupported Gemma4 layer type '{value}'"
					))),
					None => Err(Error::Config(
						"Gemma4 layer_types entries must be strings".to_string(),
					)),
				})
				.collect::<Result<Vec<_>>>()?,
			Some(_) => {
				return Err(Error::Config(
					"Gemma4 layer_types must be an array".to_string(),
				));
			}
		...
		for (name, value) in [
			("rope_parameters.full_attention", full_rp),
			("rope_parameters.sliding_attention", sliding_rp),
		] {
			if let Some(value) = value
				&& !value.is_object()
			{
				return Err(Error::Config(format!("{name} must be an object")));
			}
		}
		...
		Ok(Gemma4Config {
			...
			num_global_key_value_heads: optional_i32(cfg, "num_global_key_value_heads")?,
			...
			final_logit_softcapping: optional_f32(cfg, "final_logit_softcapping")?,
			tie_word_embeddings: get_bool(cfg, "tie_word_embeddings", true)?,
			layer_types,
			full_attention_rope_theta: match full_rp {
				Some(rope) => get_f32(rope, "rope_theta", 1_000_000.0)?,
				None => 1_000_000.0,
			},
			full_attention_partial_rotary_factor: match full_rp {
				Some(rope) => get_f32(rope, "partial_rotary_factor", 0.25)?,
				None => 0.25,
			},
			sliding_attention_rope_theta: match sliding_rp {
				Some(rope) => get_f32(rope, "rope_theta", 10_000.0)?,
				None => 10_000.0,
			},
		})
	}
...
impl Rope {
	...
	fn proportional(dims: i32, theta: f32, partial_rotary_factor: f32) -> Result<Self> {
...
impl Attention {
	...
	#[allow(
		...
		reason = "attention forward keeps cache and shared-KV authority explicit"
	)]
	fn forward(
		&self,
		x: &Array,
	...
	) -> Result<(Array, Array, Array, i32, i32)> {
		let shape = x.shape();
		...
		let (keys, values, offset, key_start) = if let Some((k, v, off, start)) = shared_kv {
			(k, v, off, start)
		} else {
			let cache = cache
				.ok_or_else(|| Error::Model("gemma4: KV-owning layer missing cache".into()))?;
			let k_proj = self.k_proj.as_ref().ok_or_else(|| {
				Error::Model("gemma4: KV-owning layer missing key projection".into())
			})?;
			...
			let v = if self.use_k_eq_v {
				...
				let v_proj = self.v_proj.as_ref().ok_or_else(|| {
					Error::Model("gemma4: KV-owning layer missing value projection".into())
				})?;
			...
			let k_norm = self.k_norm.as_ref().ok_or_else(|| {
				Error::Model("gemma4: KV-owning layer missing key normalization".into())
			})?;
		...
		Ok((out, keys, values, offset, key_start))
	}
}
...
impl Block {
	...
	#[allow(
		clippy::too_many_arguments,
		reason = "decoder-layer forward keeps cache and per-layer inputs explicit"
	)]
	fn forward(
		&self,
		x: &Array,
		...
		let (h, keys, values, offset, key_start) = self.self_attn.forward(&h, cache, shared_kv)?;
		...
		Ok((h, keys, values, offset, key_start))
	}
}
...
impl Gemma4Model {
	...
	/// (which has its own chunking in [`crate::engine::media::audio`]).
	...
	pub fn new_caches(&self) -> Vec<LayerCache> {
		// Sliding-window owning layers retain only their window (plus
		// growth slack) - their mask never attends further back, and
		// full retention grows KV memory without bound over a long
		// session (see models/cache.rs).
		(0..self.num_cache_layers())
			.map(|index| match self.config.layer_types[index] {
				LayerType::Sliding => {
	...
	/// `crate::engine::generate::Session::encode_chat_with_media`).
	...
	/// `crate::engine::generate::Session::encode_chat_with_media`). Video frames
	...
	pub fn forward_with_media(
		...
		let h = self.media_embeddings(input_ids, images, audios, Cancellation::disabled())?;
		self.forward_from_embeds(input_ids, h, caches)
	}
	...
	pub(crate) fn forward_with_media_cancellable(
		...
		let h = self.media_embeddings(input_ids, images, audios, cancellation)?;
				...
				for chunk in &clip.chunks {
					...
					let features = match &audio.encoder {
						...
						AudioEncoder::Unified { .. } => {
							...
							// windows (see
							// `crate::engine::media::audio::preprocess_audio_bytes_raw`);
	...
	fn forward_from_embeds(
		...
		let per_layer_inputs: Vec<Option<Array>> = if self.config.hidden_size_per_layer_input > 0 {
			let ple = self
				.embed_tokens_per_layer
				.as_ref()
				.ok_or_else(|| Error::Model("gemma4: per-layer input embedding is missing".into()))?
				.forward(input_ids)?;
			...
			let proj = self
				...
				.as_ref()
				.ok_or_else(|| {
					Error::Model("gemma4: per-layer model projection is missing".into())
				})?
				.forward(&h)?;
			...
			let proj = self
				...
				.as_ref()
				.ok_or_else(|| {
					Error::Model("gemma4: per-layer projection normalization is missing".into())
				})?
				.forward(&proj)?;
		...
		for (idx, layer) in self.layers.iter().enumerate() {
			...
			let (h_out, keys, values, offset, key_start) = if idx_i32 < self.first_kv_shared_layer {
				...
				let shared = intermediates[source]
					.clone()
					.ok_or_else(|| Error::Model(format!("gemma4: KV source {source} not ready")))?;
				layer.forward(&h, None, Some(shared), per_layer_input)?
			};

			intermediates[idx] = Some((keys, values, offset, key_start));
			h = h_out;
		}
