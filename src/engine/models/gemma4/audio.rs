...
//!   `audio_tower.layers.{i}.self_attn.{post.(linear.)weight,
//! relative_k_proj.weight, per_dim_scale}`   `audio_tower.layers.{i}.
//! norm_{pre_attn,post_attn,out}.weight`   `audio_tower.layers.{i}.lconv1d.
//! {pre_layer_norm,linear_start,depthwise_conv1d,conv_norm,linear_end}`
...
use super::{
	super::config::{get_bool, get_f32, get_i32},
	vision::ClippedLinear,
};
use crate::engine::{
	array::{Array, Dtype},
	error::Result,
...
impl AudioConfig {
	pub fn from_json(cfg: &Value) -> Result<Self> {
		Ok(AudioConfig {
			hidden_size: get_i32(cfg, "hidden_size", 1024)?,
			num_hidden_layers: get_i32(cfg, "num_hidden_layers", 12)?,
			num_attention_heads: get_i32(cfg, "num_attention_heads", 8)?,
			conv_kernel_size: get_i32(cfg, "conv_kernel_size", 5)?,
			rms_norm_eps: get_f32(cfg, "rms_norm_eps", 1e-6)?,
			attention_context_left: get_i32(cfg, "attention_context_left", 13)?,
			attention_logit_cap: get_f32(cfg, "attention_logit_cap", 50.0)?,
			attention_invalid_logits_value: get_f32(cfg, "attention_invalid_logits_value", -1e9)?,
			residual_weight: get_f32(cfg, "residual_weight", 0.5)?,
...
impl AudioAttention {
	...
	fn build_mask(&self, t: i32) -> Result<Array> {
		let tu = t as usize;
	...
	fn build_distance_indices(&self, t: i32) -> Result<Array> {
		let tu = t as usize;
