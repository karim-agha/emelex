...
use super::super::config::{get_f32, get_i32};
use crate::engine::{
	array::Array,
	error::Result,
	nn::{LayerNorm, Linear, WeightMap},
	ops,
};
...
impl UnifiedVisionConfig {
	pub fn from_json(cfg: &Value) -> Result<Self> {
		Ok(UnifiedVisionConfig {
			patch_size: get_i32(cfg, "patch_size", 16)?,
			pooling_kernel_size: get_i32(cfg, "pooling_kernel_size", 3)?,
			mm_embed_dim: get_i32(cfg, "mm_embed_dim", 3840)?,
			mm_posemb_size: get_i32(cfg, "mm_posemb_size", 1120)?,
			num_soft_tokens: get_i32(cfg, "num_soft_tokens", 280)?,
...
/// `crate::engine::media::image::preprocess_image_bytes`) `model_patch_size x
...
impl UnifiedVisionEmbedder {
	...
	/// `crate::engine::media::image::preprocess_image_bytes`'s alignment).
...
/// shaped `[n, 2]` int32 (row-major, matching
/// `UnifiedVisionEmbedder::patchify`).
fn build_positions(grid_h: i32, grid_w: i32) -> Result<Array> {
