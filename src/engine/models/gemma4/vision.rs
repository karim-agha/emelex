...
//! position_embedding_table}`   `vision_tower.encoder.layers.{i}.
//! {input_layernorm,...}.weight`   `vision_tower.encoder.layers.{i}.self_attn.
//! {q,k,v,o}_proj.(linear.)weight`   `vision_tower.encoder.layers.{i}.
//! self_attn.{q,k}_norm.weight`   `vision_tower.encoder.layers.{i}.mlp.{gate,
//! up,down}_proj.(linear.)weight`   `embed_vision.embedding_projection.*`

use serde_json::Value;

use super::super::config::{get_bool, get_f32, get_i32};
use crate::engine::{
	array::{Array, Dtype},
	error::{Error, Result},
	nn::{Linear, RmsNorm, WeightMap},
	ops,
};
...
impl VisionConfig {
	pub fn from_json(cfg: &Value) -> Result<Self> {
		let rope = cfg
			...
			.map(|value| {
				value.as_object().ok_or_else(|| {
					Error::Config("vision rope_parameters must be an object".to_string())
				})?;
				Ok::<&Value, Error>(value)
			})
			.transpose()?;
		let rope_theta = match rope {
			Some(rope) => get_f32(rope, "rope_theta", 100.0)?,
			None => 100.0,
		};
		...
		Ok(VisionConfig {
			...
			num_hidden_layers: get_i32(cfg, "num_hidden_layers", 16)?,
			num_attention_heads: get_i32(cfg, "num_attention_heads", 12)?,
			num_key_value_heads: get_i32(cfg, "num_key_value_heads", 12)?,
			head_dim: get_i32(cfg, "head_dim", 64)?,
			rms_norm_eps: get_f32(cfg, "rms_norm_eps", 1e-6)?,
			patch_size: get_i32(cfg, "patch_size", 16)?,
			position_embedding_size: get_i32(cfg, "position_embedding_size", 10240)?,
			default_output_length: get_i32(cfg, "default_output_length", 280)?,
			pooling_kernel_size: get_i32(cfg, "pooling_kernel_size", 3)?,
			use_clipped_linears: get_bool(cfg, "use_clipped_linears", false)?,
			rope_theta,
		})
	}
}
...
fn build_positions(patch_h: i32, patch_w: i32) -> Result<Array> {
	let num_patches = (patch_h * patch_w) as usize;
