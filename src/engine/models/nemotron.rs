...
use super::{
	base::{attention_mask_for, merge_heads, split_heads},
	cache::{GatedDeltaCache, KvCache, LayerCache},
	config::{finite_f32, get_bool, get_f32, get_i32, get_str, require_i32},
	mamba2::{Mamba2Config, Mamba2Mixer},
};
use crate::engine::{
	array::Array,
	error::{Error, Result},
...
impl NemotronConfig {
	pub fn from_json(cfg: &Value) -> Result<Self> {
		...
		let layers = pattern
			...
			.map(|c| match c {
				...
				other => Err(Error::Config(format!(
					"nemotron_h: unsupported layer type '{other}' in \
					 hybrid_override_pattern (MoE 'E' not yet implemented)"
				))),
			})
		...
		let rms_norm_eps = get_f32(cfg, "rms_norm_eps", 1e-5)?;
		let layer_norm_eps = get_f32(cfg, "layer_norm_epsilon", rms_norm_eps)?;
		let (time_step_min, time_step_max) = match cfg.get("time_step_limit") {
			Some(Value::Array(values)) if values.len() == 2 => (
				finite_f32(&values[0], "time_step_limit[0]")?,
				finite_f32(&values[1], "time_step_limit[1]")?,
			),
			Some(_) => {
				return Err(Error::Config(
					"time_step_limit must be a two-number array".to_string(),
				));
			}
